...
# calls run() to invoke the binary via run(), then asserts on
# RUN_OUT / RUN_EXIT. run() wraps env -i PATH="$PATH" for a clean
...
run() {
    ...
    RUN_EXIT=0
    env -i PATH="$PATH" "$X" "$@" > "$TMPOUT" 2> "$TMPERR" || RUN_EXIT=$?
    RUN_OUT=$(< "$TMPOUT")
    RUN_ERR=$(< "$TMPERR")
    rm -f "$TMPOUT" "$TMPERR"
}
...
test_define_crunch_merges_consecutive_delimiters() {
    run define -d , -C -s FOO "a,,b" "$ECHO" '$FOO'
    assert_exit "exits_ok" 0
    assert_eq "merged" "a b" "$RUN_OUT"
}

test_define_custom_delimiter() {
    run define -d , -s FOO "a,b" "$ECHO" '$FOO'
    assert_exit "exits_ok" 0
...
test_importas_default_on_missing_var() {
    run importas -D default FOO UNDEFINED_VAR "$ECHO" 'hello $FOO'
    assert_exit "exits_ok" 0
    assert_eq "output" "hello default" "$RUN_OUT"
}

test_importas_insist_fails_on_missing_var() {
    run importas -i FOO UNDEFINED_VAR "$ECHO" hello
...
test_forstdin_iterates_lines() {
    ...
    printf 'a\nb\n' > "$F"
    WORK_DIR="$F"
    run forstdin VAR importas VAL VAR "$ECHO" '$VAL' < "$F"
    assert_exit "exits_ok" 0
    assert_eq "both_lines" $'a\nb' "$RUN_OUT"
}

test_forbacktickx_iterates_generator_output() {
    run forbacktickx VAR "$ECHO" -e 'a\nb' '' importas VAL VAR "$ECHO" '$VAL'
    assert_exit "exits_ok" 0
    assert_eq "both_values" $'a\nb' "$RUN_OUT"
}

test_elglob_expands_pattern() {
    run elglob MYVAR '*.c' "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_elglob_no_match_unset_var() {
    run elglob MYVAR 'does_not_exist_*.xyz' importas -D defaulted VAL MYVAR "$ECHO" '$VAL'
    assert_exit "exits_ok" 0
...
test_eltest_logical_and_false() {
    run eltest -d / -a -f /nonexistent_xyz
...
test_getcwd_stores_current_directory() {
    run cd / getcwd MYPATH importas VAL MYPATH "$ECHO" '$VAL'
    assert_exit "exits_ok" 0
    assert_eq "root" "/" "$RUN_OUT"
}

test_heredoc_feeds_inline_data() {
    run heredoc 0 hello tr '[:lower:]' '[:upper:]'
    assert_exit "exits_ok" 0
    assert_eq "uppercased" "HELLO" "$RUN_OUT"
}

test_heredoc_multiline_data() {
    run heredoc 0 'ab' tr -d '\n'
    assert_exit "exits_ok" 0
    assert_eq "joined" "ab" "$RUN_OUT"
}

test_fdmove_dups_fd() {
    run fdmove 1 2 "$ECHO" ok 2>/dev/null
    assert_exit "exits_ok" 0
}

test_fdmove_closes_old_fd() {
    run fdreserve 7 fdmove -c 7 8 "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_fdclose_closes_fd() {
    run fdclose 9 "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_fdreserve_reserves_fd() {
    run fdreserve 7 "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_fdswap_swaps_two_fds() {
    run fdreserve 7 fdreserve 8 fdswap 7 8 "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_fdblock_sets_blocking() {
    run fdblock 0 "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_posix_cd_changes_directory() {
    run posix-cd / "$ECHO" ok
    assert_exit "exits_ok" 0
}

test_multidefine_substitutes_multiple_keys() {
    run multidefine A prefixA B prefixB -- "$ECHO" '$A/$B'
    assert_exit "exits_ok" 0
    assert_eq "joined" "prefixA/prefixB" "$RUN_OUT"
}

test_multisubstitute_applies_multiple_defines() {
    run multisubstitute define A x define B y '' "$ECHO" '$A $B'
    assert_exit "exits_ok" 0
...
test_envfile_loads_vars_from_file() {
    ...
    echo "FOO=bar" > "$F"
    WORK_DIR="$F"
    run envfile "$F" importas VAL FOO "$ECHO" '$VAL'
    assert_exit "exits_ok" 0
...
test_tryexec_continues_after_failed_prog() {
    run tryexec /nonexistent_exec_dne_xyz "$ECHO" fallback
...
test_trap_handles_signal() {
    run trap USR1 "$ECHO" caught '' getpid PP foreground importas VAL PP kill -USR1 '$VAL' ''
    assert_exit "exits_ok" 0
    assert_contains "action_ran" "$RUN_OUT" "caught"
}

test_execlineb_parses_simple_script() {
    run execlineb -c 'echo hello world'
    assert_exit "exits_ok" 0
    assert_eq "output" "hello world" "$RUN_OUT"
}

test_execlineb_expands_dollarat() {
    run execlineb -c 'echo $@' a b
    assert_exit "exits_ok" 0
    assert_eq "output" "a b" "$RUN_OUT"
}
